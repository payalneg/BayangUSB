...
void Bayang_init()
{
    ...
    delay(20);
    NRF24L01_FlushRx();
    uint8_t setup = NRF24L01_ReadReg(NRF24L01_03_SETUP_AW);
    if (setup >= 1 && setup <= 3) {
      #ifdef DEBUG  
      Serial.println("Bayang_init OK");
      #endif    
    }
    else
    {
      #ifdef DEBUG 
      Serial.print("Bayang_init Fail ");
      Serial.println(setup);
      #endif    
      while (1)
      {
        digitalWrite(LED_pin_connected, 1);
        digitalWrite(LED_pin, 0);
        delay(100);
        digitalWrite(LED_pin_connected, 0);
...
void Bayang_bind()
{
    ...
    while(bindPackets < 1200 && consecutiveNoData < 1000) {
        ...
        while(millis() < timeout) 
        {
            ...
            { 
                ...
                {
                  if ((packet[0] == 0xA4)||(packet[0] == 0xA3)||(packet[0] == 0xA2)||(packet[0] == 0xA1))
                  {
                    if (0 == bindPackets)
                    {
    ...
    Serial.print(Bayang_rx_tx_addr[0]); 
    Serial.print(Bayang_rx_tx_addr[1]);
    Serial.print(Bayang_rx_tx_addr[2]);
    Serial.print(Bayang_rx_tx_addr[3]);
    Serial.println(Bayang_rx_tx_addr[4]);
    Serial.print("rf_channel: ");
    Serial.print(Bayang_rf_channels[0]);
    Serial.print(Bayang_rf_channels[1]);
    Serial.print(Bayang_rf_channels[2]);
    Serial.println(Bayang_rf_channels[3]);
    ...
    { 
      ...
      if (checkPacket(packet))
      {
        if (packet[0] == 0xA5)
        {
          ...
          #ifdef DEBUG
          char oooo[64];
          sprintf(oooo, "aux: %i %i %i %i",aux[CH_INV],aux[CH_FLIP],aux[CH_HEADFREE],aux[CH_RTH]);     
          Serial.print(oooo);     
          #endif
          //all good
          lastRxTime = recTime;
